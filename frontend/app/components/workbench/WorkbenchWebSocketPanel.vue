...
<template>
  <div class="flex h-full min-h-0 flex-col overflow-hidden bg-card/10 select-none">
    <div class="flex h-10 shrink-0 items-center justify-between border-b bg-muted/30 px-4">
      <div class="flex items-center gap-3">
        ...
        <div class="min-w-0">
          ...
          <span class="mt-1 block font-mono text-[8px] font-bold uppercase tracking-widest text-muted-foreground/40 leading-none">Outbound Frame Constructor</span>
        </div>
      </div>

      <div 
        class="flex h-6 items-center px-2 border-2 font-mono text-[9px] font-black uppercase tracking-widest transition-all"
        :class="workbench.webSocketState.value === 'connected' ? 'border-primary/30 bg-primary/10 text-primary shadow-[0_0_10px_rgba(16,185,129,0.1)]' : 'border-border/60 text-muted-foreground/40'"
      >
    ...
    <div class="flex h-14 shrink-0 items-center justify-between bg-muted/20 px-4">
      <div class="flex items-center gap-2 font-mono text-[9px] font-bold uppercase tracking-widest text-muted-foreground/40 italic">
      ...
      <button
        ...
        :class="canSend 
          ...
          : 'border-border/40 bg-muted/10 text-muted-foreground/30 cursor-not-allowed'"
        type="button"
        :disabled="!canSend"
        @click="sendMessage"
      >
